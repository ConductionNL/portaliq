...
- [~] **T05**: Read declared collections via OpenRegister's objects API scoped by subject + Organisation; render list/detail. — CODE COMPLETE, live-integration probe pending. `PortalObjectReader` implemented against the **verified** OR 0.2.17 API (`ObjectService::findAll(['filters'=>['register'=>…,'schema'=>…,<scopeField>=>subjectRef,'organisation'=>…]])` — the old `findAll(register:,schema:)` named-arg form is gone, confirming the drift). Filters on the collection's declared `scopeField` (= subjectRef) + re-verifies every row (drops foreign-subject rows; defense in depth) + graceful-degrades to `[]` without OR. Guarded endpoint `GET /portal/api/collections/{register}/{schema}` authorises the (register,schema) against the subject's own manifest before reading. React renders a per-collection loader. Unit-tested (scoping/drop-foreign/degrade). REMAINING: verify anonymous portal reads actually resolve against a live OR — portal subjects are not NC users, so OR RBAC/multitenancy behaviour for a `#[PublicPage]` context needs a live probe (may surface OR-side work).
