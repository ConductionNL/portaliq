...
This page gets a working Portaliq running on your own machine in two commands. You end with a themed public portal you can open without logging in.
...
```bash
curl -fsSLO https://raw.githubusercontent.com/ConductionNL/portaliq/development/portaliq-compose.yaml
```
...
```bash
docker compose -f portaliq-compose.yaml logs -f app-installer
```
...
==> apps present: integriq openregister portaliq thematiq
```
...
```bash
curl -s http://localhost:8613/status.php
```
...
| What | Where |
| --- | --- |
| **Public portal** | [http://localhost:8613/apps/portaliq/site?portal=demo](http://localhost:8613/apps/portaliq/site?portal=demo) |
| **Portaliq** | [http://localhost:8613/apps/portaliq/](http://localhost:8613/apps/portaliq/) |
| Admin interface | [http://localhost:8613](http://localhost:8613) — `admin` / `admin` |
...
| `portaliq` | The app this page is about. |

That OpenRegister dependency is **not declared** in `appinfo/info.xml` — no app in the fleet declares an `<app>` dependency — so nothing stops the App Store from installing portaliq without it. It would then load, find no register to attach to, and show you an empty app rather than an error. The compose file encodes the dependency the manifest does not.
...
curl -s "http://localhost:8613/apps/portaliq/api/content/site?portal=demo"
...
curl -s -u admin:admin "http://localhost:8613/apps/openregister/api/registers" | head -c 300
```
...
```bash
# Stop, keep the data
docker compose -f portaliq-compose.yaml down

# Stop and delete everything, including the database
docker compose -f portaliq-compose.yaml down -v
```
...
**Everything returns 404 or a maintenance page after a restart.** Nextcloud is waiting for an upgrade. Run `docker compose -f portaliq-compose.yaml exec -u www-data nextcloud php occ upgrade`.
