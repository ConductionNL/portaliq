...
test.describe('site renderer — multi-site', () => {
	test('S5: two websites publish the same route and do not leak into each other', async ({ request }) => {
		...
		const tilburg = await request.get(`${API}/page?route=/over-ons&site=open-tilburg`)
		const venray = await request.get(`${API}/page?route=/over-ons&site=open-venray`)
	...
	test('S5b: a page listing carries only its own site', async ({ request }) => {
		const tilburg = await (await request.get(`${API}/pages?site=open-tilburg`)).json()
		const venray = await (await request.get(`${API}/pages?site=open-venray`)).json()
	...
	test('S6: a domain serves once verified and not before — both directions', async ({ request }) => {
	...
	test('S7: an unknown host resolves to nothing and reveals nothing', async ({ request }) => {
	...
	test('S7b: a named site that does not exist does not fall through to the host', async ({ request }) => {
	...
	test('S6b: the API reports a distinct theme REFERENCE per site', async ({ request }) => {
		...
		const tilburg = await (await request.get(`${API}/site?site=open-tilburg`)).json()
		const venray = await (await request.get(`${API}/site?site=open-venray`)).json()
