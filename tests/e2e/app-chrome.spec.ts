...
test.describe('app chrome (ADR-114)', () => {
	...
	test('Store opens the hosted store surface, which this app writes no backend for', async ({
		...
		await expect(page).toHaveURL(/\/apps\/portaliq\/store(\?|$)/, {
			timeout: 15_000,
		})

		// The page is declarative: openregister hosts the store plane, so
		// portaliq ships NO store controller (ADR-080, ADR-114 Decision 4).
		// With no registry configured it renders the app's own items and makes
		// NO network call, so this must pass on a plain instance.
		await expect(page.locator('[data-testid="cn-nav"]')).toBeVisible()
	})
