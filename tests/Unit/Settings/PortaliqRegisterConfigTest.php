...
class PortaliqRegisterConfigTest extends TestCase {
	...
	public function testRegisterJsonParsesAndVersionsAreBumped(): void {
		...
		// Additive.
		// 0.25.0 (portal 0.7.0): `traffic` declares `widget: "json"`, so the
		// block renders in the portal form at all. `fieldsFromSchema` drops
		// an object property carrying neither a widget nor a $ref, which is
		// why measurement could not be switched on from the interface. Its
		// own version because 0.24.0 was already taken by the schema-binding
		// fix: a change that shares a version with one already imported never
		// re-imports, and the field would have stayed missing.
