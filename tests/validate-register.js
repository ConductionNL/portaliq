...
function appNamespace() {
	...
	if (!m) return null
	// app id like "openconnector" or "portaliq" → namespace "OpenConnector" / "Portaliq"
	const camel = m[1]
		.split(/[-_]/)
...
// Schema slugs the app's REAL descriptors define — the set a mock's objects
// must resolve against. Cached per run.
let realSchemaSlugs = null
function realSchemaSlugSet() {
	if (realSchemaSlugs) return realSchemaSlugs
	realSchemaSlugs = new Set()
	for (const f of registerFiles()) {
		let reg
		try {
			reg = JSON.parse(fs.readFileSync(f, 'utf8'))
		} catch {
			continue
		}
		const xo = reg['x-openregister']
		if (xo && xo.type === 'mock') continue
		const schemas = (reg.components && reg.components.schemas) || {}
		for (const [name, s] of Object.entries(schemas)) {
			realSchemaSlugs.add(name)
			if (s && typeof s.slug === 'string') realSchemaSlugs.add(s.slug)
		}
	}
	return realSchemaSlugs
}

// The contract of a `type: mock` descriptor: no schema copies, the real
// register's slug, and only objects whose register/schema resolve.
function validateMockRegister(reg, label, errors) {
...
function validateRegister(file, errors, warnings) {
	...
	// A `type: mock` descriptor (generated by hydra-gates
	// generate_mock_register.py) deliberately carries NO schema definitions:
	// it declares the app's real register and demo objects, and OpenRegister
	// resolves each object's schema by slug against what the real descriptor
	// installed. A copy of the schemas in the mock is what WOO-558 was about
	// (imported under the demo's own config identity it became a second,
	// parallel schema set the register never linked). So the mock is judged
	// on its own contract instead of the real descriptor's.
	if (xo && xo.type === 'mock') {
		validateMockRegister(reg, label, errors)
		return
	}
